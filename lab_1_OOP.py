...
class Human:
    ...
    Например, Человек Дима, 1999 года рождения, мужского пола. Может добавить,что он работает преподавателем, имеет рост- 175, вес - 70.
    """
...
class Monster(ABC):
    """ Класс "Монстер" (абстрактный базовый класс) имеет такие характеристики, как: силу, вид, часть тела, которой он атакует, его местонахождение в пространстве, атаку. Мы можем увеличивать его силу, атаковать противника.
    """
...
class Bat(Monster):
    """ Класс "Летучая мышь". Имеет характеристики базового класса.(силу, вид, часть тела, которой он атакует, его местонахождение в пространстве, атаку). Помимо этого умеет летать.
     Например, летучая мышь, имеющая тип - "черная",  с силой 100, которая атакует противника зубами, находится на таких координатах : х=0, у =0, z = 0. Она может укусить противника, тогда на экран выведется "Кусь". И может  изменять свое положение по осям с помощью метода fly.
...
class Werewolves(Monster):
    """ Класс "Оборотень". Имеет характеристики базового класса.(силу, вид, часть тела, которой он атакует, его местонахождение в пространстве, атаку). Помимо этого умеет прыгать.
         Например, оборотень вида тривиального,с силой 55, который атакует противника когтями, находится на таких координатах : х=0, у =0, z = 0. Она может укусить противника, тогда на экран выведется "ррррр".
         И может  изменять свое положение по осям с помощью метода jump
         Werewolves('ordinary',55,'K','claw',0,0,0) """
    def attack(self) -> None:
        print('rrrr')
